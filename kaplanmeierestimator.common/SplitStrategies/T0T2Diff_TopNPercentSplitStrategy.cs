...
{
    public class T0T2Diff_TopNPercentSplitStrategy : ISplitStrategy
    {
        private readonly int m_percent;
        private readonly IDictionary<int, Patient> m_patients;

        public T0T2Diff_TopNPercentSplitStrategy(int percent, IDictionary<int, Patient> patients)
        {
        ...
        {
            var relevantGenes = genes.Where(gene => !double.IsNaN(gene.AbsoluteDifference) && m_patients.ContainsKey(gene.PatientId));
            var orderedGenes = relevantGenes.OrderBy(gene => gene.AbsoluteDifference);
