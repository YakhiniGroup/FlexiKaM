...
{
    public class T2_TopNPercentSplitStrategy : ISplitStrategy
    {
        private readonly int m_percent;
        private readonly IDictionary<int, Patient> m_patients;

        public T2_TopNPercentSplitStrategy(int percent, IDictionary<int, Patient> patients)
        {
        ...
        {
            var relevantGenes = genes.Where(gene => !double.IsNaN(gene.After) && m_patients.ContainsKey(gene.PatientId));
            var orderedGenes = relevantGenes.OrderBy(gene => gene.After);
